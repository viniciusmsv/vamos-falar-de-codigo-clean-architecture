...
from src.adapter.api.promoters import store_controllers as store_controller
from src.adapter.api.promoters import user_controllers as user_controller
...
def create_app(injector: Injector) -> FastAPI:
    ...
    app.include_router(
        dog_facts_controller.router, prefix="/api/v1/dogs", tags=["dogs"]
    )
    app.include_router(
        cat_facts_controller.router, prefix="/api/v1/cats", tags=["cats"]
    )
    app.include_router(
        store_controller.router,
        prefix="/api/v1/promoters/store",
        tags=["store"]
    )
    app.include_router(
        user_controller.router,
        prefix="/api/v1/promoters/user",
        tags=["user"]
    )
